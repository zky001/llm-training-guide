...
| K3 · 三种找法 | 关键词/向量/混合检索、BM25、RRF 融合（1 个交互实验） | ✅ |
| K4 · 重排 | 召回-精排两阶段、交叉编码器、ColBERT（1 个交互实验） | ✅ |
| K5 · 生成与引用 | 上下文拼装、有据回答、引用标注（1 个交互实验） | ✅ |
| K6 · 评测 | RAG 三元组、三大失败模式（1 个交互实验） | ✅ |
| K7 · 进阶与前沿 | GraphRAG、Agentic RAG、RAG vs 长上下文（2 个交互实验） | ✅ |
